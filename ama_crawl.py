...
from tkinter import Tk, Label, Button, Text, END, RAISED, NW, Toplevel, Frame, messagebox
...
class Tkshoot(Tk):
    ...
    def __init__(self):
        Tk.__init__(self)
        self.title('亚马逊产品调查表')
        self.geometry('1100x600+500+100')
...
class Control:
    def __init__(self, master, boxes, func):
        ...
        for num in range(7):
            self.x = 50
            self.y = 10 + 60 * num
            self.label = Label(self.parent, text='网址%s:' % str(2*num+1),
                               ).place(x=self.x+30, y=self.y, anchor=NW)
            self.box1 = Text(self.parent,
                             width=50,
                             height=2,
                             bd=2.1)
            self.box1.place(x=self.x+100, y=self.y, anchor=NW)
            self.label2 = Label(self.parent,
                                text='网址%s:' % str(2*num+2),
                                ).place(x=11*self.x+30, y=self.y, anchor=NW)
            self.box2 = Text(self.parent,
                             width=50,
                             height=2,
                             bd=2.1)

            self.box2.place(x=11*self.x+100, y=self.y, anchor=NW)
        ...
        self.btn = Button(self.parent, text="下载Excel", relief=RAISED, command=func).place(x=440, y=480, anchor=NW)
        self.btn2 = Button(self.parent, text="清空链接", relief=RAISED, command=clear_links).place(x=540, y=480, anchor=NW)
    ...
    def show_pic(self, box, x_p, y_p):
        try:
            ...
            if link:
                ...
                self.parent.images.append(tk_image)
                label = Label(self.parent, image=tk_image, width=w_re, height=h_re)
                label.image = tk_image
                label.place(x=x_p, y=y_p, anchor=NW)
                label2 = Label(self.parent)
                label.bind("<Enter>", self.handlerAdaptor(self.view_large, label=label2, pic=img, x_p=x_p, y_p=y_p))
                label.bind("<Leave>", self.handlerAdaptor(self.hide_view, label=label2))
                timer = Timer(1, self.judge_change, (box, label, link, x_p, y_p))
                timer.daemon = True
                timer.start()
            else:
    ...
    def view_large(self, event, label, pic, x_p, y_p):
        w_box, h_box = 400, 400
        w, h = pic.size
        ...
        label.configure(image=tk_image, width=w_re, height=h_re)
        label.image = tk_image
        if y_p + h_re >= 580:
            label.place(x=x_p + 100, y=580-h_re)
        else:
            label.place(x=x_p + 100, y=y_p + 50)

    def hide_view(self, event, label):
        label.place_forget()

    def judge_change(self, box, label, link, x_p, y_p):

        try:
            link2 = box.get('1.0', END).strip()
            if link != link2:
                label.destroy()
                timer = Timer(2, self.show_pic, (box, x_p, y_p))
                timer.daemon = True
                timer.start()
            else:
                timer = Timer(1, self.judge_change, (box, label, link, x_p, y_p))
...
class TreadMain():
    def __init__(self, boxes, master):
        ...
        for i, box in enumerate(self.boxes):
            if i / 2 == i // 2:
                timer = Timer(1, self.gui.show_pic, (box, 20,  5 + 60 * i/2))
                timer.daemon = True
                timer.start()
            else:
                timer = Timer(1, self.gui.show_pic, (box, 520, 5 + 60 * (i-1)/2))
                timer.daemon = True
                timer.start()
